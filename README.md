...
Как было выяснено ранее, для хранении данных пользователей понадобится порядка 20 Тб, а для хранения твитов - 12 Пб.
...
```
12 Пб / 5 Тб = 2,4к, учитывая запас на рост хранлища, хранения логов, метрик и прочего - 6к бакетов
```

| Amazon S3 | Количество бакетов|
|-|-|
| Хранилище данных| 6 000 |
...
Используя данные статьи [^8], можно сделать вывод, что PostgreSQL может выдерживать порядка 40к TPS. Не все запросы к базе данных будут использовать транзакции, поэтому вполне вероятно, что несколько реплик с такой конфигурацией справятся с нагрузкой.

| CPU | RAM (ГБ) | Диск (ТБ) | Количество |
|-|-|-|-|
| 12 | 128 | 5 | 15 - на лайки, 10 - на посты, 5 - на пользователей |
...
Nginx может выдерживать порядка 50к RPS [^9]. Для обеспечения отказоустойчивости и надежности предусмотрим несколько реплик.
...
[^8]: [PostgreSQL and MySQL: Millions of Queries per Second](https://www.percona.com/blog/2017/01/06/millions-queries-per-second-postgresql-and-mysql-peaceful-battle-at-modern-demanding-workloads/)
[^9]: [Optimizations: Tuning Nginx for better RPS of an HTTP API](https://rohitgupta.xyz/blog/tuning-nginx-for-better-rps-of-an-http-api/)
